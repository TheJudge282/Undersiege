...
function BlockSmoke
{
	...
	my.size = random(100)+100;
 	my.bmap = AtomSmoke;

	my.move = 0;
	my.alpha = 15;
	my.function = BlockSmokeFunction;
...
function  WallParts()
{
 ...
 my.pan-=90;
 my.skill20 = GetVertexNum(me);
 while (my.frameanim<500)
 {
 
 my.skill1 += 1;
 my.frameanim += my.skill1;
 ent_frame( "frame", my.frameanim);
	my.skill21 = 0;
	while (my.skill21<my.skill20)
	{
		if (random(10)>8)
		{
		
			vec_for_vertex( temp2, my, my.skill21);
			temp.x = random(1)-0.5;
			temp.y = random(1)-0.5;
			temp.z = (random(1)+0.5)*10;
			effect( BlockSmoke, 1, temp2, temp);
		}
	 my.skill21+=1;
	}
wait(1);
