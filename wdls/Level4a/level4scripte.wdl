...
action arrow1
{
	...
	while (1)
	{
		...
		if ((player.y < -700)&&(player.x > -1920))||((player.y > -700)&&(player.x > -1450))
		{
			ShowMessage(5,142);
			Play_My_Sounds(3,0);
			my.skill27 = 0;
			while (my.skill27 < 250)
			{
				my.pan += 10*(speed/200);
				if (Game_State[0] == 4)
				{
...
action car_path1	//for part one 
{
	...
	else
	{
		...
		if (game_state[0] <5)
		{
			if (ent_nextpoint(my._target_x) > 15)
			{
...
{	
	...
	{
		if (eplhealth){	return;	}
		if (game_state[1] == 1)&&(game_state[0] < 4)
		{
			my._signal=0;
			car_stop();
			return;
