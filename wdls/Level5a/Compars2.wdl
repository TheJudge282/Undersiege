...
function SolderDie()
{
	...
	hSolderPointer[my.LeaderId] = 0;
	
	//nshowmessage("died");
	hSoldierCount[hAxcelNumber[my.LeaderId] -1] -=1;
	
	if (hSoldierCount[hAxcelNumber[my.LeaderId] -1] <= 0)
	{
		iCounter = int(random(4));
		CreateRandomSold(iCounter);
		//test_num2 = 0;
		//test_num3 = 0;
		//test_num4 = 0;
		//test_num6 = 0;
...
function solderevent()
{
 ...
 if (event_type  == event_entity)&&(indicator == CarAccedant)
 {
	...
	{ 
		if (str_cmpni(level_str,"level52h.WMB") == 0)
		{
