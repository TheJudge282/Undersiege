...
function HeartPlayer(HV)
{
	...
	if (entiy._health<-100)
	{
		...
		SetActivePart( SharedEntity, BodyParts[4],0);
...
//				SetBoneParam( SharedEntity,"body Neck");
		...
		if (Camra2Player)
		{
			...
			if (mouse_force.x > 0.5)&&(AbsForce.y == 0)&&(AbsForce.x == 0)&&(PlaCru == 0)&&(lader_player == 0)
			{
				if ((weapon_firing != 1)&&(weapon_reloading != 1))
				{
			...
			if (((my_angle.pan > 0.3)||(my_angle.pan < -0.3))&&(AbsForce.y == 0)&&(AbsForce.x == 0)&&(lader_player == 0)) 
				||((NSonar(Pointer_To_Entity(my.BodyLnk))!=0)&&(lader_player == 0))
			{
				SetActiveBlendSeq( SharedEntity, BodyParts[4],"standerd","standerd");
				if (grnade_firing == 0)
				{SetActiveSeq( SharedEntity,  BodyParts[PartIndex],"turn");
			...
			else
			{
				if(lader_player == 0)
				{
					...
					if (abs(NSonar( Pointer_To_Entity(my.BodyLnk))>6))&&(lader_player == 0)
				...
				{
					SetActivePart( SharedEntity, BodyParts[0],1);
					SetActiveSeq( SharedEntity,  BodyParts[0],"1ammo");
					AnimModelPart( SharedEntity, AbsForce*50, BodyParts[0]);
				}
			}
