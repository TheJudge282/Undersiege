...
{	
	...
	if (random(10)>5)
	{
		play_entsound( me, MetalHit1, __BullVollll);
		goto fing;
	}
	if (random(10)>5)
	{
		play_entsound( me, MetalHit2, __BullVollll);
		goto fing;
	}
	if (random(10)>5)
	{
		play_entsound( me, MetalHit3, __BullVollll);
		goto fing;
	}
	play_entsound( me, MetalHit4, __BullVollll);
	fing :
	my.skill1 = result;
...
function ShotEffect()
{
	...
	}else
	{
		...
		if (you._type == TypeOfEngines)
		{
			   effect( Bullete_Smoke, 10, target, temp);
			   effect( Metal_Parts, 10, target, normal);
			   create(<point1.mdl> , you.pos , MetalBullete);
			   return;
		}
	  if (bloodenable)
	  {
