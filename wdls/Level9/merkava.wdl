	 ...
	 if (my._ShotPazoka==0)
	  {
		...
		if ( EDistance( point_move, me)<100) 
		{
...
while (1)
{
	if (snd_playing(pTankSound) == 0)
	{
		play_entsound(me,TankSound,1000);
		pTankSound = result;
	}
	 ...
	 if (my._ShotPazoka==0)
	  {
		...
		temp.z = 0;
		vec_to_angle( my_angle, temp);
		my.pan = nearangle( my.pan, my_angle, speed/25);
		my.flag2 = abs(ang(my.pan - my_angle))>5;
		temp.x = my.force;
		temp.y = 0;
		sonar me, 40000;
