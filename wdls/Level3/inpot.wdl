...
function WalkForword(Body)
{
	...
	if (lader_player == 0)
	{
		if(PressedKeys[Key_StrafLeft] == 0)&&(PressedKeys[Key_StrafRight] == 0)
		{
			...
			{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"walk","walk");
			}else
			{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"run","run");
			}
		}
	}
	else
	{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"climb","climb");
	}
}
...
function keyFWalk()
{
	...
	while (key_pressed(lkey)==1)//&&(grnade_firing == 0)
	{
		if (grnade_firing == 0)
...
function WalkBackword(Body)
{
	...
	if (lader_player == 0)
	{
		if (pressedKeys[Key_StrafLeft] == 1)
		{
			...
			BoneParam[0] = 20;
			SetBoneParam( Body,"body Neck");
			SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"back walk","back walk");
		}
		else
		{
			...
			{
				SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"back walk","walk");
			}
		}
	}
	else
	...
	}else
	{
		...
		else
		{
			...
			else
			{
				...
				BoneParam[0] = 0;
				SetBoneParam( Body,"body Neck");
				SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"ShimySide2","ShimySide2");
			}
		}
	}
}
...
function KeySLeft()
{
	...
	while (key_pressed(lkey)==1)&&(lader_player == 0)
	{
		if (grnade_firing == 0)
...
function WalkRight(Body)
{
	...
	else
	{
		...
		{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"ShimySide1","ShimySide1");
		}
	}
}
...
function KeySRight()
{
	...
	while (key_pressed(lkey)==1)&&(grnade_firing == 0)&&(lader_player == 0)
	{
...
function TurnLeft(Body)
{
	...
	{
		if (PressedKeys[Key_StrafLeft] == 1)
		{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"walk","WALK LEFT");
		}else
		{
			if (PressedKeys[Key_StrafRight] == 1)
			{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"walk","WALK RIGHT");
			}else
			{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"walk","rotateL");
			}
		}
	}
}
...
function JumpingDestance(Body)
{
	...
	if (player)
	{
		if (NSonar( Pointer_To_Entity(player.BodyLnk))==0)
		{
			...
			{
				SetActivePart( Body, BodyParts[PartIndex-8],1);
...
function Jumpingrame(Body)
{
	...
	if (grnade_firing == 0)
	{SetActiveBlendSeq( Body, BodyParts[PartIndex-8],"run","jump");
	}
}
function KeyJump()
{
	if (grnade_firing != 0)||(lader_player != 0)
	{return;
	}
	if (freeze_mode)
		{return;}
