...
if (weapon)
{
CurrentAmmo.x = int(totalbullets[weapon._ammotype - 1]/ maxbulletnum[weapon._ammotype - 1]+ 0.5);
...
function initSound()
{
 ...
 SliderBtn.y = SliderBtn.skill2-soundvolume*Lowp/100;
 SetVolume( CurrentSong,  musicvolume);

}


function SvaeTheNotSaveyesyorno()
{
	while (1)
	{
		if (key_y)
		{
			Save_PathPoints("nn.pts");
			wait(3);
			exit;
...
VisMainMenu(0);
	...
	AddPath("music");
	//if (level.z == 2)
	//{modelindex = Load3dMdel("mariem.012");
//	}
//	else
//	{
...
//		if (SplashCounter == 1)
//		{
//			morph scr52,splashscreen;
//			stop_sound(StartSoundH);
//			play_sound(SSound2,100);
//			StartSoundH = result; 
//			
//		}
//		
//		if (SplashCounter == 2)
//		{
//			morph scr53,splashscreen;
//			morph PressEnter2b,PreesEnter;
//			stop_sound(StartSoundH);
...
//	}
	...
	//{
		...
		if (SplashCounter == 1)
		{
			morph SplashScreen2,splashscreen;
			stop_sound(StartSoundH);
			play_sound(SSound2,100);
			StartSoundH = result; 
