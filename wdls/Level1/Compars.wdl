...
function ShotOnHim(V1,v2)
{
	...
	while (my._state == MyState)
	{
		...
		wait(Weap_fire_time[ my.WeaponNumber]*2);
...
function ShotM16(V1,v2)
{
	...
	while (my._state == MyState)
	{
		...
		{
			if (random(10)>5)
				{play_entsound(me,m16fire1,10000);}
			else
				{play_entsound(me,m16fire2,10000);}
		}
		temp.x = speed/2;
		...
		wait(Weap_fire_time[ my.WeaponNumber]);
		you = Pointer_To_Entity( my.EnemyId);
		if (vec_dist( my.x , you.x)>1200)
			{wait(10);}
		else
...
function AttackThenHide()
{
 ...
 my._state = _state_attack;
	ShotM16(GunFlashFront[my.skill6],GunFlashBack[my.skill6]);
	get_nextH_point(me);
...
CRt :
 ...
 create( <cksnp.mdl>, temp, SolderSniper);
 } 
 remove me;
}
...
function SSolder()
{
 ...
 if (SnipersCount.y<20)
{
 temp.x = 3103.760;
 temp.y = 1078.292;
 temp.z = 213.635;
 //create( <cksnp.mdl>, temp, SniperCreation);
...
/*	if event_type == event_scan && indicator == _gunfire && (my._state == _state_wait || my._state == _state_Walk )
	...
	if event_type == event_detect && you._type == _type_player1
	{
		my.EnemyId = Entity_To_Pointer( you);
	...
	if event_type == event_detect && indicator == CloseShot && you._type != _type_player1 && you._state == _state_attack
	{
		 my.AnimShot = IDownSnipFire;
		 return;
	}
...
while (my._state == _state_wait)
{
	...
	{ 
		...
		if (SoldCheckIfHealthy())
		{ 
		        my.LastState = Function_To_Pointer( "SolderWait");
			return;
		}
