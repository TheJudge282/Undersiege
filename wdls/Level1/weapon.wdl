...
//				goto StepNull;
			...
			if (my._ammotype == 6)
			{SetActiveSeq( SharedEntity,  BodyParts[PartIndex],WeaponFrames[4]);
			}else
			{SetActiveSeq( SharedEntity,  BodyParts[PartIndex],WeaponFrames[2]);
			}
			AnimModelPart( SharedEntity, 50, BodyParts[PartIndex]);
...
return;
	...
	my._fire = 10;
	     ...
	     if (weapon._ammotype == 6)					  { play_sound(s99_empty_sound ,50);	}			
...
{
	if (WeaponN == colt)             { play_entsound( me, colt_fire_sound, 100);		goto Snd;}
	if (WeaponN == ak47)            { play_entsound( me, ak_fire_sound,100);                goto Snd;}
	if ( (WeaponN == m16) ||(WeaponN == m4))
					{ play_entsound( me, m16_fire_sound, 100);		goto Snd;}
	if (WeaponN == pumaction)        { play_entsound( me, pumaction_fire_sound, 100);	goto Snd;}
	if (WeaponN == s99sniper)        { play_entsound( me, s99_fire_sound, 1000);		goto Snd;}
	if (WeaponN == lmg)             { play_entsound( me, lmg_fire_sound, 1000);		goto Snd;}
	if (WeaponN == uzi)              { play_entsound( me, uzi_fire_sound, 100);		goto Snd;}
	if (WeaponN == rbj)              { play_entsound( me, rbj_fire_sound, 100);		goto Snd;}
	if (WeaponN == galili)           { play_entsound( me, galili_fire_sound, 100);		goto Snd;}
	if (WeaponN == Doubleuzi)       { play_entsound( me, uzi_fire_sound, 100);		goto Snd;}
	if (WeaponN == 15)               { play_entsound( me, Stick_fire, 100);			goto Snd;}
	//if (WeaponN == 16)               { play_entsound( me, gurnade_fire, 100);                 return;}
	Snd :
	temp.x = speed/2;
...
function zoom()
{
   ...
   if (  weapon.Zoomable)
	...
	InitPPCamera();
	   ...
	   CPointer.scale_x = 0.011;
	   CPointer.scale_y = 0.011;
	   if (weapon.Zoomable==30)
	   {
	    CPointer.x = 10; goto con;
	   }
	   if (weapon.Zoomable==15)
	   {
	    CPointer.x = 20.5; goto con;
	   }
	   if (weapon.Zoomable==20)
	   {
