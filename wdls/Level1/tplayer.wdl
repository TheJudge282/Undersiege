...
function FailMessages()
{
	...
	var rn;
	rn = int(random(3));
	if (rn == 0){ showmessage(10,147); } // 16
	if (rn == 1){ showmessage(10,140); } // 9
	if (rn == 2){ showmessage(10,139); } // 8
	if (rn == 3){ showmessage(10,146); } // 15
	Play_My_Sounds(5,rn);
}
...
function HeartPlayer(HV)
{
	...
	if (entiy._health<-100)
	{
		...
		// hani code
		preparecamera();
		my.entity2 = Pointer_To_Entity(my.WeaponMesh1);
		remove(my.entity2);		
...
//-----------------------------------------------------------------
// hani code
function scanfromfingure(witchhand)
{
	// right hand
	if (witchhand!=1)
	{
		vec_for_vertex(temp,player,429);
		if (vec_dist( temp, goldi_pointer.x)<=25)
		{
	...
	if (witchhand!=2)
	{
		vec_for_vertex(temp,player,453);
		if (vec_dist( temp, goldi_pointer.x)<=25)
		{
...
function KickAttac()
{
	...
	SetActivePart( SharedEntity, BodyParts[0],1);
	SetActiveSeq( SharedEntity,  BodyParts[0],"weapon");
	while (FrameTime < 27)
	{
		...
		if (FrameTime.y!=3)
		{
			FrameTime.y +=  scanfromfingure(FrameTime.y);
		}
		//----------------------------------------------------------------------
		wait(1);
	}
