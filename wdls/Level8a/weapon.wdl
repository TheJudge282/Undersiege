...
{     		
	...
	if (my._ammotype == colt)                { play_sound(Scolt_fire_sound,30);      goto Snd;	}
...
function CarryWeapon()
{
	...
	if (SpriteWeapon == 1) { sprite_weapon();	weapon.invisible = 1;	VWeaponSprite.visible = player.invisible==1;}
				...
				{weapon.invisible = 1-player.invisible;
				}
				//weapon.invisible = 1-player.invisible;
...
function WeaponFire()
{
	...
	// luai level8
	if (weapon_firing == 1)||(DecetPlayer == 1)
	{
		return;
	}
	if (weapon._weaponnumber != 1)&&(BellAct.y == 0)
	{BellAct.y = 1;
	}
	// end luai
	me = weapon;
