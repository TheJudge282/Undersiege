...
{
	if (player != null)&&(level.z == 1)
	{
		if (my.z > 600)&&(player.z < 500)
		{my._health -=1000;
		}
	}
	if (player != null)&&(level.z == 2)&&(my._type == _type_soldier)
	{
		if (my.z > -100)&&(player.z < -200)&&(Ahmed_Point > 16)&&(Ahmed_Point < 20)
		{my._health -=1000;
		}
		if (my.z < -200)&&(player.z > -100)&&(Ahmed_Point > 48)
		{my._health -=1000;
		}
	}
	if (my._health <=-200)
...
function ded_event()
{
	if ((event_type == event_scan)&&(indicator == _handle)&&(you._type == _type_player1))
	{
		...
		if ((you  == Pointer_To_Entity(player.BodyLnk))||( result==0 ))
		{
			if(my.flag7 == 0)&&(playerAct.x == 0)
			{
				playerAct.x = 1;
				my.flag7=1;
				my.skill7 = my.z;
...
function ded_soldier()
{
	my._type = _type_actor;
